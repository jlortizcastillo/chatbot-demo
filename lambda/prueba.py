...
def get_duration(appointment_type):
    appointment_duration_map = {'gynecology': 30, 'pediatrics': 60, 'ophthalmology': 30}
...
def validate_book_appointment(appointment_type, date, appointment_time):
    if appointment_type and not get_duration(appointment_type):
        return build_validation_result(False, 'AppointmentType', 'I did not recognize that, can I book you a gynecology, pediatrics, or ophthalmology?')
...
def build_options(slot, appointment_type, date, booking_map):
    ...
    if slot == 'AppointmentType':
        return [
            {'text': 'gynecology (30 min)', 'value': 'gynecology'},
            {'text': 'pediatrics (60 min)', 'value': 'pediatrics'},
            {'text': 'ophthalmology (30 min)', 'value': 'ophthalmology'}
        ]
    elif slot == 'Date':
